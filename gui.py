...
class MorseGUI:
    ...
    def __init__(self, root: tk.Tk, config: Config, decoder: Decoder, audio):
        self.root = root
        self.config = config
    ...
    def _build_output(self):
        frame = ttk.LabelFrame(self.root, text="Decoded Output", padding=6)
    ...
    def _build_visualizer(self):
        frame = ttk.LabelFrame(self.root, text="Carrier", padding=4)
        frame.pack(fill="x", padx=10, pady=(0, 4))
        self._visualizer = CarrierVisualizer(
            frame, self._audio,
            bg=_BG, color_on=_ACCENT, color_mid=_BORDER,
        )
    ...
    def _build_controls(self, parent):
        ...
        self._add_int_row(
            frame, 1, "Sidetone (Hz)", 200, 1200, self.config.sidetone_hz,
        ...
        # Input mode
        ttk.Label(frame, text="Input Mode").grid(
        ...
        cb_input = ttk.Combobox(
            frame, textvariable=self._input_mode_var,
            values=[m.name for m in InputMode], state="readonly", width=18,
        )
        cb_input.grid(row=4, column=1, sticky="w")
        cb_input.bind(
            "<<ComboboxSelected>>",
            lambda _: setattr(
        ...
        # Paddle layout
        ttk.Label(frame, text="Paddle Layout").grid(
        ...
        cb_layout = ttk.Combobox(
            frame, textvariable=self._paddle_layout_var,
            values=[m.name for m in PaddleLayout], state="readonly", width=18,
        )
        cb_layout.grid(row=5, column=1, sticky="w")
        cb_layout.bind(
            "<<ComboboxSelected>>",
            lambda _: setattr(
